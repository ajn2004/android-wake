...
### Phase 3: Validation
- [ ] Add unit tests for MAC validation logic and duplicate detection.
- [ ] Add unit tests for machine filtering by current SSID.
- [ ] Add integration test(s) for gating logic (approved vs non-approved network state).
- [ ] Add integration test(s) for add/remove approved SSID behavior.
- [ ] Add integration test(s) for add machine and wake-action enablement.
- [ ] Run full test suite and verify passing in CI/local environment.
...
- [ ] Core tests for validation, gating, and machine association pass.
